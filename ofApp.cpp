...
void ofApp::update() {
	...
	for (int y = -300; y <= 300; y += 300) {
		...
		auto noise_value = ofNoise(y, ofGetFrameNum() * 0.01);
		rotation = glm::rotate(glm::mat4(), ofMap(noise_value, 0, 1, -360, 360) * (float)DEG_TO_RAD, glm::vec3(0, 1, 0));

		this->setRingToMesh(this->face, this->frame, glm::vec3(0, y, 0), 0, 50, 10, rotation);
		for (auto radius = 60; radius <= 190; radius += 30) {

			glm::highp_mat4 rotation;
			auto noise_value = ofNoise(y, (ofGetFrameNum() + radius) * 0.003);
			...
			this->setRingToMesh(this->face, this->frame, glm::vec3(0, y, 0), radius, 28, 10, rotation);
		}
	}
}
...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float width, float height, glm::highp_mat4 rotation) {
	...
	for (int deg = 45; deg < 400; deg += deg_span) {
		...
		vertices.push_back(glm::vec3((radius * 3 + width * 0.5) * cos(deg * DEG_TO_RAD), (radius + width * 0.5) * sin(deg * DEG_TO_RAD), height * -0.5));
		vertices.push_back(glm::vec3((radius * 3 + width * 0.5) * cos((deg + deg_span) * DEG_TO_RAD), (radius + width * 0.5) * sin((deg + deg_span) * DEG_TO_RAD), height * -0.5));
		vertices.push_back(glm::vec3((radius * 3 + width * 0.5) * cos((deg + deg_span) * DEG_TO_RAD), (radius + width * 0.5) * sin((deg + deg_span) * DEG_TO_RAD), height * 0.5));
		vertices.push_back(glm::vec3((radius * 3 + width * 0.5) * cos(deg * DEG_TO_RAD), (radius + width * 0.5) * sin(deg * DEG_TO_RAD), height * 0.5));
